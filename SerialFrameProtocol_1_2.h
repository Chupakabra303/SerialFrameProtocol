...
#ifndef FRAME_BUFF_SIZE
#define FRAME_BUFF_SIZE 128
#endif

#define LOG_CMD '~' // префикс для logPrint / logWrite
...
class SerialFrameProtocol : public Print {
  public:
    ...
    template <typename T> size_t write(T *pt, size_t size) { return write((const uint8_t *)pt, size); }
    template <typename T> size_t write(T val) { return write((const uint8_t *)&val, sizeof(val)); }
    inline void frameStart() { frameCheck.init(); }
    void frameStart(const __FlashStringHelper *f) { frameStart((const char *)f); }
    template <typename... T> void frameStart(T... vals);
    void frameEnd();
    void framePrintCallback(void (* customFrame)());
    void framePrintCallback(void (* customFrame)(SerialFrameProtocol &));
    //void framePrint(void (* customFrame)(SerialFrameProtocol &)) { frameWrite(customFrame); } // алиас
    template <typename... T> void frameWrite(T...);
    template <typename... T> void framePrint(T...);
    bool frameReceiver();
    void frameClear(bool clear = true);

/* --------------- Custom protocol API ------------------ */
    template <typename... T> void logPrint(T...);
    template <typename... T> void logWrite(T...);
    uint8_t cmdReceiver();
    void cmdClear();
    uint8_t cmdQueue;
    size_t cmdLength;
...
template <typename... T> void SerialFrameProtocol::frameStart(T... vals) {
  frameStart();
  write(vals...);
}
...
void SerialFrameProtocol::framePrintCallback(void (* customFrame)()) {
  frameStart();
  customFrame();
  frameEnd();
}

void SerialFrameProtocol::framePrintCallback(void (* customFrame)(SerialFrameProtocol &sfp)) {
  frameStart();
  customFrame(*this);
  frameEnd();
}

template <typename... T> void SerialFrameProtocol::framePrint(T... vals) {
  frameStart();
  print(vals...);
  frameEnd();
}

template <typename... T> void SerialFrameProtocol::frameWrite(T... vals) {
  frameStart();
  write(vals...);
  frameEnd();
}
...
/*
template <typename T> size_t SerialFrameProtocol::write(T *pt, size_t size) {
  // return write((const uint8_t *)pt, size);
  size_t n = 0;
...
/* --------------- Custom protocol API ------------------ */
template <typename... T> void SerialFrameProtocol::logPrint(T... vals) {
  frameStart(LOG_CMD);
  print(vals...);
  frameEnd();
}

template <typename... T> void SerialFrameProtocol::logWrite(T... vals) {
  frameStart(LOG_CMD);
  write(vals...);
  frameEnd();
}
